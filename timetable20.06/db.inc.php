...
public static function getFilteredLockedUsers(array $filters = []): array
{
    ...
    public static function updateUserProfile(int $userId, array $fields): void
    {
        ...
        {
            $pdo = self::db_connect();
            return $pdo->query('SELECT id, day_name FROM weekdays ORDER BY id')->fetchAll(PDO::FETCH_ASSOC);
        }
        ...
        {
            $pdo = self::db_connect();
            return $pdo->query('SELECT id, start_time, end_time FROM time_slots ORDER BY id')->fetchAll(PDO::FETCH_ASSOC);
        }
        ...
        public static function saveUserSchedule(int $userId, array $schedule): void
        {
            ...
            try {
                ...
                $pdo->rollBack();
                self::getLogger()->info('Fehler beim Speichern des Stundenplans', ['user_id' => $userId, 'error' => $e->getMessage()]);
                throw $e;
            }
        }
        ...
        // === Helferfunktionen (z. B. db_connect, fetchOne, getLogger) müssen ebenfalls existieren ===
    }
